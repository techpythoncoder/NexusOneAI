...
from search_service.core.database import Base, get_db
...
# Mock OpenSearch integrations
@pytest.fixture(autouse=True)
def mock_opensearch():
    with mock.patch("search_service.routes.search.search", new_callable=mock.AsyncMock, return_value={"hits": []}) as mock_search, \
         mock.patch("search_service.routes.search.index_document", new_callable=mock.AsyncMock) as mock_index, \
         mock.patch("search_service.routes.search.delete_document", new_callable=mock.AsyncMock) as mock_delete:
        yield {
            "search": mock_search,
