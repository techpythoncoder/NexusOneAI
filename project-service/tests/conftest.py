...
from project_service.core.database import Base, get_db
...
# Engine setup scoped to session
@pytest.fixture(scope="session")
...
# Mock Kafka producer to avoid local cluster dependency
@pytest.fixture(autouse=True)
def mock_kafka():
    mock_prod = mock.AsyncMock()
    with mock.patch("project_service.core.kafka.get_producer", return_value=mock_prod) as mock_get:
        yield mock_get

# Mock search index calls to prevent network timeouts and warnings
@pytest.fixture(autouse=True)
def mock_search():
    with mock.patch("project_service.core.search._call", new_callable=mock.AsyncMock) as mock_call:
        yield mock_call

# Async client for executing requests against the test application
@pytest.fixture
async def client():
    # Use standard httpx client with ASGI transport for FastAPI
